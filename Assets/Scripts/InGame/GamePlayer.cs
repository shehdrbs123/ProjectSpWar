...
public class GamePlayer : MoveObject {
	...
	public Skill[] skills;
	public UserUIButton DimensionSupportTarget;
	public SkillButton skillbuttonPressed=null;
	void Start(){
		if(!isServer)
	...
	void Update () {
		...
		if (skillbuttonPressed != null) {
			if(skills[skillbuttonPressed.ID] == null) return;
			if (DimensionSupportTarget != null) {
				CmdSkillAction (DimensionSupportTarget.gameObject,skillbuttonPressed.ID);
				DimensionSupportTarget.AfterResetButton ();
			} else {
				Debug.Log (skills [skillbuttonPressed.ID].coolTime);
				CmdSkillAction (null, skillbuttonPressed.ID);
			}
			skillbuttonPressed.cooltime = skills [skillbuttonPressed.ID].coolTime;
			StartCoroutine (skillbuttonPressed.coolTimeCoroutine ());
			skillbuttonPressed = null;
		}
	}
		
	[Command]
	public void CmdSkillAction(GameObject target,int ID){//OnStartServer에서 스킬 개인 설정 해야함. 
		if(skills[ID] == null) return;
		if (target == null) {
			GameObject skillIns = (GameObject)Instantiate (skills[ID].gameObject, bulletSpawn[0].position, bulletSpawn[0].rotation);
			NetworkServer.Spawn(skillIns);
		} else {
			GameObject skillIns = (GameObject)Instantiate (skills[ID].gameObject, target.transform.position, target.transform.rotation);
			NetworkServer.Spawn(skillIns);
		}
	}
}
